...
export default function DocumentUpload() {
  ...
  const renderUploadBox = (typeKey, title, maxText, icon, color) => {
    ...
    if (doc) {
      ...
      return (
        <div style={{ background: isVerified ? '#ecfdf5' : '#f8fafc', border: `1px solid ${isVerified ? '#a7f3d0' : '#e2e8f0'}`, borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '140px', textAlign: 'center' }}>
    ...
    return (
      <div 
        ...
        style={{ border: `2px dashed ${color === 'purple' ? '#d8b4fe' : '#cbd5e1'}`, borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '140px', background: color === 'purple' ? '#fcfaff' : '#ffffff', cursor: isUploading ? 'not-allowed' : 'pointer', transition: 'all 0.2s', textAlign: 'center' }}
        onMouseOver={(e) => e.currentTarget.style.borderColor = color === 'purple' ? '#a855f7' : '#94a3b8'}
        onMouseOut={(e) => e.currentTarget.style.borderColor = color === 'purple' ? '#d8b4fe' : '#cbd5e1'}
      >
        <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#f3e8ff', color: '#714b9c', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.75rem' }}>
          {isUploading ? (
        ...
        <p style={{ margin: 0, fontWeight: 600, color: '#1e293b', fontSize: '0.9rem' }}>{title}</p>
        <p style={{ margin: '0.25rem 0 0 0', color: '#64748b', fontSize: '0.75rem' }}>{maxText}</p>
      </div>
    );
  };
  ...
  const getBadge = (doc, defaultType) => {
    if (!doc) {
      ...
      return <span style={{ background: '#f1f5f9', color: '#64748b', padding: '0.25rem 0.6rem', borderRadius: '12px', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.05em' }}>PENDING</span>;
    }
    ...
    <Layout>
      <div style={{ padding: '1rem 2rem', maxWidth: '1200px', margin: '0 auto', fontFamily: '"Inter", "Segoe UI", sans-serif', background: '#faf8fc', minHeight: '100%', position: 'relative' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#111827', margin: '0 0 0.5rem 0' }}>Document Verification</h1>
          <p style={{ color: '#4b5563', fontSize: '0.95rem', margin: 0 }}>Please upload clear, legible copies of your identification and education documents to complete your onboarding process.</p>
        </div>

        {/* Profile Alert */}
        {!profileReady && (
          <div style={{ background: '#f5edfd', borderRadius: '12px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', border: '1px solid #e9d5ff' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: '#714b9c', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '2px' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, color: '#714b9c', fontSize: '0.85rem' }}>Some document fields are locked until your basic profile information is verified by the HR team.</p>
              </div>
            </div>
            <Link to="/profile" style={{ background: '#714b9c', color: 'white', padding: '0.6rem 1.25rem', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none', flexShrink: 0, transition: 'background 0.2s' }}>
              View Profile
            </Link>
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          {/* Aadhar Card */}
          <div style={{ background: '#ffffff', borderRadius: '16px', padding: '1.5rem', border: '1px solid #f1f5f9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#f5edfd', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="24" height="24" fill="none" stroke="#714b9c" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"></path></svg>
              </div>
              {getBadge(getDocStatus('aadhar'), 'REQUIRED')}
            </div>
            <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.25rem', fontWeight: 700, color: '#1e293b' }}>Aadhar Card</h3>
            <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.85rem', color: '#64748b', lineHeight: 1.4, flex: 1 }}>Identity proof. Front &amp; back merged.</p>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', padding: '1.5rem', border: '1px solid #f1f5f9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column' }}>
            ...
            <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.25rem', fontWeight: 700, color: '#1e293b' }}>PAN Card</h3>
            <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.85rem', color: '#64748b', lineHeight: 1.4, flex: 1 }}>Income tax identity document.</p>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', padding: '1.5rem', border: '1px solid #f1f5f9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column' }}>
            ...
            <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.25rem', fontWeight: 700, color: '#1e293b' }}>Degree Certificate</h3>
            <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.85rem', color: '#64748b', lineHeight: 1.4, flex: 1 }}>Latest degree or post-grad certificate.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
          ...
          <div style={{ gridColumn: '1 / -1', '@media(minWidth: 1024px)': { gridColumn: 'span 2' }, background: '#ffffff', borderRadius: '16px', padding: '1.5rem', border: '1px solid #f1f5f9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'row', gap: '2rem', flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 300px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#f5edfd', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
                  <svg width="24" height="24" fill="none" stroke="#714b9c" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.25rem', fontWeight: 700, color: '#1e293b' }}>Passport Photo</h3>
                <p style={{ margin: '0 0 1.5rem 0', fontSize: '0.9rem', color: '#64748b', lineHeight: 1.5 }}>Please provide a high-resolution, front-facing photograph with a white background. This will be used for your official ID card and internal portal.</p>
              </div>
              <div style={{ flex: '1 1 250px', display: 'flex', flexDirection: 'column' }}>
                <div 
                  onClick={() => fileInputRefs.passport.current?.click()}
                  style={{ flex: 1, border: '2px dashed #d8b4fe', borderRadius: '12px', background: '#fcfaff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', minHeight: '200px', transition: 'all 0.2s', position: 'relative', overflow: 'hidden' }}
                  onMouseOver={(e) => e.currentTarget.style.borderColor = '#a855f7'}
                  onMouseOut={(e) => e.currentTarget.style.borderColor = '#d8b4fe'}
                >
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#f3e8ff', color: '#714b9c', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.75rem' }}>
                    ...
                    <span style={{ fontWeight: 600, color: '#1e293b' }}>Upload Photo</span>
                    <span style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '0.25rem' }}>JPG, PNG up to 5MB</span>
                  </div>
                </div>
        ...
        {/* Toast Notification */}
        {toast && (
          <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', background: '#302c38', color: 'white', padding: '1rem 1.25rem', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', gap: '1rem', zIndex: 50, animation: 'slideIn 0.3s ease-out' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: toast.type === 'success' ? '#8b5cf6' : '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {toast.type === 'success' ? (
            ...
            <button onClick={() => setToast(null)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', marginLeft: '0.5rem' }}>
