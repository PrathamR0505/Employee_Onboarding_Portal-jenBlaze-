  ...
  return (
    <Layout>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 700, color: '#404E3B', marginBottom: '0.25rem' }}>HR Overview</h1>
          <p style={{ fontSize: '0.95rem', color: '#404E3B' }}>Welcome back. You have {pendingCount || 12} pending onboarding tasks today.</p>
        </div>
        <Link to="/hr/invite" style={{ textDecoration: 'none' }}>
          <button style={{ background: '#7B9669', color: '#fff', border: 'none', padding: '0.75rem 1.25rem', borderRadius: '8px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
      ...
      <div className="grid grid-4" style={{ marginBottom: '2rem', gap: '1.5rem' }}>
        <div className="card" style={{ padding: '1.5rem', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
            <div style={{ width: '40px', height: '40px', background: '#BAC8B1', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7B9669' }}>
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#10b981', background: '#BAC8B1', padding: '0.2rem 0.5rem', borderRadius: '999px' }}>+12%</span>
          </div>
          <div style={{ color: '#404E3B', fontSize: '0.9rem', fontWeight: 500, marginBottom: '0.25rem' }}>Total Employees</div>
          <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#404E3B' }}>{totalEmployees > 0 ? totalEmployees : '1,284'}</div>
        </div>
        ...
        <div className="card" style={{ padding: '1.5rem', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ color: '#404E3B', fontSize: '0.9rem', fontWeight: 500, marginBottom: '0.25rem' }}>Pending Verifications</div>
          <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#404E3B' }}>{pendingCount || '48'}</div>
        </div>
        ...
        <div className="card" style={{ padding: '1.5rem', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ color: '#404E3B', fontSize: '0.9rem', fontWeight: 500, marginBottom: '0.25rem' }}>Completed Onboarding</div>
          <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#404E3B' }}>{completedCount || '156'}</div>
        </div>
        ...
        <div className="card" style={{ padding: '1.5rem', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ color: '#404E3B', fontSize: '0.9rem', fontWeight: 500, marginBottom: '0.25rem' }}>Rejected Documents</div>
          <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#404E3B' }}>{rejectedCount}</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', border: '1px solid #f3f4f6', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', marginBottom: '2rem' }}>
        <div style={{ padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f3f4f6' }}>
          <h2 style={{ fontSize: '1.25rem', color: '#404E3B', fontWeight: 700 }}>Employees List</h2>
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <button style={{ background: '#BAC8B1', border: '1px solid #9AA991', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#374151', cursor: 'pointer' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {overview.map((emp, idx) => (
                <tr key={emp.id} style={{ borderBottom: '1px solid #f3f4f6', background: '#BAC8B1' }}>
                  <td style={{ padding: '1rem 1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: ['#BAC8B1', '#fce7f3', '#BAC8B1', '#dcfce7'][idx % 4], color: ['#7e22ce', '#be185d', '#7B9669', '#15803d'][idx % 4], display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: '0.9rem', flexShrink: 0 }}>
                        {getInitials(emp.name)}
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <strong style={{ color: '#404E3B', fontWeight: 600, fontSize: '0.95rem', lineHeight: '1.2' }}>{emp.name.split(' ')[0]}</strong>
                        {emp.name.split(' ').slice(1).join(' ') && (
                          <span style={{ color: '#404E3B', fontSize: '0.9rem' }}>{emp.name.split(' ').slice(1).join(' ')}</span>
                        )}
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '1rem 1.5rem', color: '#404E3B', fontSize: '0.9rem' }}>{emp.email}</td>
                  <td style={{ padding: '1rem 1.5rem' }}>
                    {getStatusPill(emp.onboarding_status)}
                  </td>
                  <td style={{ padding: '1rem 1.5rem', color: '#404E3B', fontSize: '0.9rem' }}>{emp.city || '—'}</td>
                  <td style={{ padding: '1rem 1.5rem' }}>
                    <div style={{ width: '80px', height: '4px', background: '#9AA991', borderRadius: '4px' }}>
                      <div style={{ width: `${emp.progress_percent || 0}%`, height: '100%', background: '#7B9669', borderRadius: '4px' }}></div>
                    </div>
                  </td>
                  <td style={{ padding: '1rem 1.5rem', textAlign: 'center' }}>
                    <Link to={`/hr/documents?userId=${emp.id}`} style={{ color: '#7B9669', display: 'inline-flex', padding: '0.25rem', borderRadius: '4px', textDecoration: 'none' }}>
              ...
              {overview.length === 0 && (
                <tr>
                  <td colSpan="6" style={{ textAlign: 'center', padding: '3rem', color: '#404E3B', fontSize: '0.9rem' }}>No employees found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f3f4f6', background: '#E6E6E6' }}>
          <span style={{ color: '#404E3B', fontSize: '0.85rem', fontWeight: 500 }}>Total Employees Listed: {overview.length}</span>
        </div>
      </div>
